...
struct socket {
    int type; // tcp or udp
    int desc; // tcp 连接池中的 index
};

/**
 * @brief 创建一个socket文件，这个socket里面存了连接池中的index，在desc中
 * 
 * @param domain 
 * @param type 
 * @param protocol 
...
socketbind(struct socket *s, struct sockaddr *addr, int addrlen) {
    if (s->type == SOCK_STREAM)
        return tcp_api_bind(s->desc, addr, addrlen);// s->desc 实际上就是socket在连接池中的index，通过这个找到我们的socket结构体。
    else
