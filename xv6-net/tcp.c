...
struct tcp_hdr {
    ...
    uint32_t ack; // 确认号
    uint8_t  off; // 保留(没用)
    uint8_t  flg; // flags [URG,ACK,PSH,RST,SYN,FIN]
    uint16_t win; // 窗口大小
    uint16_t sum; // 检验和
    uint16_t urg; // 紧急指针
};
...
// 代表一次tcp连接，也称为一个socket结构体
struct tcp_cb {
    uint8_t used;
    uint8_t state; // tcp 状态，listen
    struct netif *iface;
    uint16_t port;// 和此socket结构体绑定的一个port
    struct {
        ip_addr_t addr;
        uint16_t port;
    } peer;
     ...
     * 
     *      单位：单位是字节
     *      una: 还未确认的序号，一般una=rcv.ack，这样向前移动，在接收时移动
     *      nxt: 允许发送的序号，一般nxt+=len，在发送时移动
     *      wnd: 窗口的大小（单位是字节），一般wnd=rcv.wind，在接收时更新
     *
     *      |--sent,acked--|--sent,unacked--|--allow to send--|--not allow to send--|
     *                     |                |                 |       
     *                    una              nxt                |
     *                     |<-----------------wnd------------>|   
     */
    ...
    struct {
        uint32_t nxt; // 见上
        uint32_t una; // 见上
        uint16_t up;  // todo: 暂时未用到
        uint32_t wl1; // todo: 暂时未用到
        uint32_t wl2; // todo: 暂时未用到
        uint16_t wnd; // 见上
    } snd;
    ...
    /**
     *      |--reveived,acked--|--allow to receive--|--unsent,allow--|
     *                         |                    |                     
     *                        nxt                nxt+wnd   
     */
    struct {
...
tcp_tx (struct tcp_cb *cb, uint32_t seq, uint32_t ack, uint8_t flg, uint8_t *buf, size_t len) {
    ...
    hdr->sum = 0;
    hdr->urg = 0;
    // 将buf中的长为len的内容拷贝到tcp报文中, tcp 报文长度为：sizeof(struct tcp_hdr) + len
    ...
    // todo: 为什么放入ip层之后还要再放入发送队列，是因为tcp可靠性传输的要求吗
...
tcp_timer_thread (void *arg) {
    ...
    while (1) {
        ...
        for (cb = cb_table; cb < array_tailof(cb_table); cb++) {
            ...
            txq = cb->txq.head;
            while (txq) {
                if (ntoh32(txq->segment->seq) >= cb->snd.una) { // 如果还没有ack
                    if (timestamp.tv_sec - txq->timestamp.tv_sec > 3) {// 如果超时
                        peer = cb->peer.addr;
                        ip_tx(cb->iface, IP_PROTOCOL_TCP, (uint8_t *)txq->segment, txq->len, &peer);// 重传
                        txq->timestamp = timestamp;// 重新计时
                    }
                    // 如果没有超时
                    // update previous tcp_txq_entry
                    prev = txq;
                    txq = txq->next;
                } else {// 如果已经ack了，应该将这个retry移除
...
tcp_incoming_event (struct tcp_cb *cb, struct tcp_hdr *hdr, size_t len) {
    ...
    switch (cb->state) {
        ...
        case TCP_CB_STATE_LISTEN:
            ...
            if (TCP_FLG_ISSET(hdr->flg, TCP_FLG_SYN)) {// 三次握手
...
tcp_api_connect (int soc, struct sockaddr *addr, int addrlen) {
    ...
    cb->rcv.wnd = sizeof(cb->window);
    cb->iss = (uint32_t)random();// todo: tcp的seq难道一开始发送的是随机的吗，它在这里进行初始化？
    tcp_tx(cb, cb->iss, 0, TCP_FLG_SYN, NULL, 0);// SYN 报文第一次握手
    cb->snd.nxt = cb->iss + 1;// 下一个序号是iss+1，可以看到这里序号的单位是报文
    // 三次握手剩余两次在哪里完成？
...
tcp_api_bind (int soc, struct sockaddr *addr, int addrlen) {
    ...
    acquire(&tcplock);
    for (cb = cb_table; cb < array_tailof(cb_table); cb++) {
        if (cb->port == sin->sin_port) { // 如果这个端口已经被占用，那么返回bind失败
            release(&tcplock);
            return -1;
        }
    }
    ...
    cb->port = sin->sin_port;// 服务端将端口绑定在socket结构体中
    release(&tcplock);
    return 0;
}
...
tcp_api_accept (int soc, struct sockaddr *addr, int *addrlen) {
    ...
    while ((entry = queue_pop(&cb->backlog)) == NULL) {// 从backlog queue中弹出一个entry
        sleep(cb, &tcplock);
    }
    backlog = entry->data;
    kfree((char*)entry);
